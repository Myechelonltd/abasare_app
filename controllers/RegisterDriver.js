...
const registerDriver = async (req, res) => {
    try {
        ...
        if (findDriver) return sendError(res, 409, "This email already exist! change email address 👍🏼", null);
...
const updatedDriver = async (req, res) => {
    try {
        ...
        const updatedDriver = await Register.findByIdAndUpdate({ _id: id }, req.body, {
            new: true,
        })
        if (updatedDriver) {
...
const changeDriverPass = async (req, res) => {
    try {
        ...
        let data = await Register.findOneAndUpdate({ _id: id },
             {password: hashPassword(password)} );
        const findeUpdateDriver = await Register.findOne({ _id: id });
        if (findeUpdateDriver) {
            return success(res,200,"Password Changed successful",findeUpdateDriver);
        }
        else {
